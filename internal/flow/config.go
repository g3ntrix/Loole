...
func NormalizeConfigText(text string) (string, bool) {
	...
	text = strings.TrimPrefix(text, "LOOLE_CONFIG=")
	text = strings.TrimSpace(text)
...
func (a AuthConfig) accessTokenForRoute(ctx context.Context, route RouteConfig) (OAuthAccessToken, error) {
	if token := strings.TrimSpace(os.Getenv("LOOLE_ACCESS_TOKEN")); token != "" {
...
func oauthTokenAttempts(route RouteConfig) []oauthTokenAttempt {
	...
	if route.Proxy == "" && route.GoogleIP != "" && shouldPinGoogleIP(route.Mode) {
		directRoute := route
		directRoute.Mode = "direct"
		directRoute.GoogleIP = ""
		return []oauthTokenAttempt{
			{
			...
			{
				source:  "refresh_token:oauth2_pinned",
				route:   route,
				host:    "oauth2.googleapis.com",
				path:    "/token",
				timeout: baseTimeout,
	...
	if route.Proxy != "" || route.GoogleIP != "" || isGoogleFrontRoute(route.Mode) || shouldPinGoogleIP(route.Mode) {
		frontRoute := route
